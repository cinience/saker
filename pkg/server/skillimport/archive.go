...
// PrepareSource resolves the import source into a local directory. For path
// sources it returns ("", nil, nil). For git/archive it clones/downloads into
// a temp directory and returns a cleanup function.
func PrepareSource(sourceType SourceType, params Params) (string, func(), error) {
	switch sourceType {
	case SourcePath:
		return "", nil, nil
	case SourceGit:
		return CloneGit(params.RepoURL)
	case SourceArchive:
		return DownloadArchive(params.ArchiveURL)
	default:
		return "", nil, fmt.Errorf("unsupported source_type %q", sourceType)
	}
}

// CloneGit clones a git repository (depth=1) into a temp directory.
func CloneGit(repoURL string) (string, func(), error) {
...
// ExtractArchive extracts a .zip or .tar.gz file into dest.
