...
func TestAgent_RepeatLoopThresholdAbortsAtConfiguredCount(t *testing.T) {
	model := &scriptedModel{
		outputs: []*ModelOutput{
			{ToolCalls: []ToolCall{{Name: "loop", Input: map[string]any{"x": 1}}}},
		},
	}
	tools := &stubTools{}
	ag, err := New(model, tools, Options{RepeatLoopThreshold: 4, MaxIterations: 50})
	if err != nil {
		t.Fatalf("new: %v", err)
	}
	_, err = ag.Run(context.Background(), NewContext())
	if err == nil || !strings.Contains(err.Error(), "4+ times") {
		t.Fatalf("expected 4+ loop error, got %v", err)
	}
...
func TestAgent_RepeatLoopDisabledWhenThresholdNegative(t *testing.T) {
	model := &scriptedModel{
		outputs: []*ModelOutput{
			{ToolCalls: []ToolCall{{Name: "loop"}}},
		},
	}
	ag, err := New(model, &stubTools{}, Options{RepeatLoopThreshold: -1, MaxIterations: 7})
	if err != nil {
		t.Fatalf("new: %v", err)
	}
	_, err = ag.Run(context.Background(), NewContext())
	if !errors.Is(err, ErrMaxIterations) {
		t.Fatalf("expected MaxIterations error (loop disabled), got %v", err)
	}
}

func TestAgent_RepeatWarningHookFiresOnceAtThresholdMinusOne(t *testing.T) {
	model := &scriptedModel{
		outputs: []*ModelOutput{
			{ToolCalls: []ToolCall{{Name: "loop", Input: map[string]any{"x": 1}}}},
		},
	...
	hook := func(_ context.Context, call ToolCall, count int) {
		if call.Name != "loop" {
			t.Errorf("hook saw unexpected call name %q", call.Name)
		}
		if call.Input["x"] != float64(1) && call.Input["x"] != 1 {
			t.Errorf("hook saw unexpected input %v", call.Input)
		}
		fires = append(fires, count)
	}

	ag, err := New(model, tools, Options{
		RepeatLoopThreshold: 5,
	...
	_, err = ag.Run(context.Background(), NewContext())
	if err == nil {
		t.Fatalf("expected loop abort error")
	}
	if len(fires) != 1 {
		t.Fatalf("hook should fire exactly once, got %d", len(fires))
	}
	if fires[0] != 4 {
		t.Fatalf("hook should fire at threshold-1=4, got %d", fires[0])
	}
}

func TestAgent_RepeatLoopDefaultThresholdIsFive(t *testing.T) {
	model := &scriptedModel{
		outputs: []*ModelOutput{
			{ToolCalls: []ToolCall{{Name: "loop"}}},
		},
	...
	tools := &stubTools{}
	// Leave RepeatLoopThreshold zero so the default kicks in.
	ag, err := New(model, tools, Options{MaxIterations: 50})
	if err != nil {
		t.Fatalf("new: %v", err)
	}
	_, err = ag.Run(context.Background(), NewContext())
	if err == nil || !strings.Contains(err.Error(), "5+ times") {
		t.Fatalf("expected default threshold=5 abort, got %v", err)
	}
	if len(tools.calls) != 5 {
		t.Fatalf("expected 5 tool calls before abort, got %d", len(tools.calls))
	}
}
