...
// clearAigoEnv unsets env vars that trigger aigo auto-registration so tool
// count assertions are stable regardless of the developer's environment.
...
func TestRegisterToolsWhitelistCaseInsensitive(t *testing.T) {
	...
	for _, want := range []string{"bash", "grep", "read", "image_read"} {
		if _, ok := seen[want]; !ok {
			t.Fatalf("missing tool %s", want)
		}
	}
