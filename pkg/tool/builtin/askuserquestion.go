...
// AskQuestionFunc is a blocking function that sends questions to the user
// and waits for their answers. Injected via context by the server layer.
type AskQuestionFunc func(ctx context.Context, questions []Question) (map[string]string, error)

type askQuestionKey struct{}

// WithAskQuestionFunc returns a context carrying the given AskQuestionFunc.
func WithAskQuestionFunc(ctx context.Context, fn AskQuestionFunc) context.Context {
	return context.WithValue(ctx, askQuestionKey{}, fn)
}

// AskQuestionFuncFromContext retrieves the AskQuestionFunc from the context, or nil.
func AskQuestionFuncFromContext(ctx context.Context) AskQuestionFunc {
	fn, _ := ctx.Value(askQuestionKey{}).(AskQuestionFunc)
	return fn
}
...
func (t *AskUserQuestionTool) Execute(ctx context.Context, params map[string]interface{}) (*tool.ToolResult, error) {
	...
	if len(answers) > 0 {
		output = formatAnsweredOutput(questions, answers)
	}
...
func formatAnsweredOutput(questions []Question, answers map[string]string) string {
	var b strings.Builder
	for _, q := range questions {
		...
		fmt.Fprintf(&b, "User has answered your questions: %q=%q. ", q.Question, answer)
	}
