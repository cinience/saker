...
func (p *progressMiddleware) AfterModel(ctx context.Context, st *middleware.State) error {
	...
	// Extract real usage from middleware state if available.
	usage := &Usage{}
	if u, ok := st.Values["model.usage"]; ok {
		if mu, ok := u.(model.Usage); ok {
			usage = &Usage{InputTokens: mu.InputTokens, OutputTokens: mu.OutputTokens}
		}
	}
...
func (p *progressMiddleware) BeforeTool(ctx context.Context, st *middleware.State) error {
	...
	iter := st.Iteration
	p.emit(ctx, StreamEvent{Type: EventToolExecutionStart, ToolUseID: call.ID, Name: call.Name, Input: call.Input, Iteration: &iter})
	return nil
}
...
func (p *progressMiddleware) AfterTool(ctx context.Context, st *middleware.State) error {
	...
	evt := StreamEvent{Type: EventToolExecutionResult, ToolUseID: call.ID, Name: call.Name, Output: payload}
	if _, hasErr := res.Metadata["error"]; hasErr {
		isErr := true
		evt.IsError = &isErr
	}
	p.emit(ctx, evt)
