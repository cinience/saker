...
| 死循环检测 | 三层检测：(1) 完全相同参数阈值（默认 5 次）立即中断；(2) 同工具硬阈值（默认 30 次）+ 缓刑期 — 注入自评 prompt 并给模型额外 3 次迭代打破模式，否则中断；(3) 高频工具（`bash`、`read`、`edit`、`grep` 等）通过 `SameToolExempt` 豁免。LLM 参与决策：注入的 prompt 要求模型说明是否在进展或应切换策略。|
...
| 动态 MCP server 转发（对象 + URI 格式） | 已支持 |
| 客户端 LLM 端点覆盖（`model_uri` 带 failover） | 已支持 |
| 系统提示词注入（prepend / append / replace） | 已支持 |
| 工具白名单 & 透传控制 | 已支持 |
...
| [AG-UI ForwardedProps](docs/zh/agui-mcp.md) | 客户端配置：model_uri、system_prompt、tools、MCP |
| [ADR](docs/adr/) | 架构决策记录 |
